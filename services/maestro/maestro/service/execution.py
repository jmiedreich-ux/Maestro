...
5. If no specialist role adequately covers a pending packet (see `specialist_role` and `support` in packets.json), do not launch it: list it in support_requests (packet_key and the specific reason). The service starts a bounded architectural-support assignment for it and notifies you when a validated role is bound; unrelated work continues. If a pending packet raises an architectural question the confirmed design does not answer, list it in architectural_questions (packet_key and the question); do not ask the Owner for an interpretation of the architecture. Use empty lists when there is nothing to request.
