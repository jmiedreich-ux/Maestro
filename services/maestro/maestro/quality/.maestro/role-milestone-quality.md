...
Catalog-bound isolated milestone QA, real data paths, evidence artifacts, cleanup/reset and retention. Own expertise for `services/maestro/maestro/quality/`; implementation is limited to the exact assigned feature paths. This role supports connected Maestro development features.

### Behavior to preserve

Follow [the controlling architecture](../../../../../docs/architecture.md#milestone-quality-assurance-and-test-data) and the registered Maestro development outcomes. Preserve service-owned records, exact versions, real evidence, essential failure handling and the separation of actor authority. Historical code comments cannot override current rules. No extra features or broader assurance gates are introduced.
...
Intended area: `services/maestro/maestro/quality/`. Current related source: No implementation of the full current QA contract is established by the indexed source tree; deterministic check helpers require separate assessment. Inspect current code and the assigned feature before deciding whether a module should move.
