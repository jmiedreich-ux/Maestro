...
def find_stale_attempts(config: RuntimeConfig, now: str) -> list[StaleAttempt]:
    ...
    what this finds. Includes the real `execution_handle`: M4.03's own
    call to `finish_attempt_execution` requires it match the attempt's
    current one exactly, or it is honestly rejected as stale.
    """
    ...
    try:
        rows = connection.execute(
            "SELECT a.attempt_id, a.packet_id, a.lease_id, a.execution_handle, "
            "a.version, p.version, l.version, l.expires_at "
            "FROM attempts a "
    ...
    return [
        StaleAttempt(
            ...
            execution_handle=str(row[3]),
            attempt_version=int(row[4]), packet_version=int(row[5]), lease_version=int(row[6]),
