...
Before initial implementation review, the Coordinator may complete a delegated worker's non-delivery and use the slice's one full review. After that review, the Coordinator may perform the sole named correction only if it remains unused, followed by the sole targeted verification. A failed targeted verification terminally returns the slice; no takeover or further review remains. Every takeover stays inside the frozen contract, paths, and gates and is reviewed by an independent non-author.

## Review-disposition routing — Owner-approved 2026-09-04

After independent implementation review, route findings to the Project
Architect before creating correction work. Dispatch a correction only for
findings marked `correct now`. For `accept known limitation`, require the
linked issue and recorded likelihood/impact rationale, preserve the unchanged
reviewed head, consume no correction, skip targeted implementation
verification, and advance it as `accepted-with-known-limitations`.

Critical exceptions, primary-outcome failures, and reserved risks do not follow
the routine known-limitation path.
