...
If no code changed, send the verified packet/PR to the independent-review queue. If code changed, send the assembled result to a different Independent Review Agent. If boundaries conflict, create a traceable `needs replan` item for the Architecture Agent/Maestro rather than attempting an unbounded repair. For committed, in-scope work that remains safe to inspect, return the complete Integration findings without requesting an isolated correction; the Coordinator combines them with the first full independent-review result under M0-D16.
