...
An applied grant makes one pending attempt eligible through the normal process checks; it does not force approval, reset counts, change scope, or satisfy unmet source/stop conditions. Publication-only recovery still uses its own counter. Registration and the architecture loop use this shared handler for their respective review/correction limits. Cancellation or restart carries the grant and consumption records for the same unresolved work.

The activity view and action result expose base limits, grants, consumption, remaining allowance, and the decision disposition. Linked free-text answers alone cannot create a grant; the typed Owner decision action applies it. Saved action receipts remain available for restart reconciliation.
