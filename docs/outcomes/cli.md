...
| Commands are invoked | Help, projects, attention, findings, retry, and exit work within this outcome's read-only scope. Help documents actual syntax and context requirements, excludes unimplemented commands, and works offline. | Each command exercised against its real path; exit leaves service work and saved records available. | None |
