...
| Outputs are validated before review | Required fields, paths, identities, dependencies, input versions, and required commits pass deterministic checks. The architect corrects precise errors within its separate allowance; unchanged errors pause early, and checks alone consume no correction or fidelity round. | Actual output correction and recheck, with assigned counts and one essential stale/misnamed-output rejection. | Accepted 2026-09-24: output corrections are counted by the run recovery allowance (two attempts), not a separate correction counter; the two allowances have the same size today. | Done 2026-09-24. Revision 00746b0 on feature/architecture-breakdown (merge revision in the handoff). Real rejections and corrections: the Codex run had one output rejected and corrected in the same conversation (`watch-b1.out` 11:29 to 11:33). The Claude run was rejected three times for `cleanup_steps` written as objects, paused with the recovery limit used (`watch-b2.out`); the architect instruction was clarified, retry accepted and the fourth output passed and published (`watch-b3.out`). Checks alone consumed no review round. After the independent review, an identical repeated error now pauses at once (`_recover_agent` in `architecture.py`); the real Claude run above predates this and used the full limit. |
