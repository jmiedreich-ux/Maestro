...
| Milestone QA or outcome review reports a defect or gap | A distinct read-only milestone-gap assignment receives exact evidence and remaining allowances. Its validated result routes implementation defects to Integration, missing in-scope work to bounded supplements, and scope changes to Owner disposition. It cannot approve or dispatch code. | Actual finding-to-determination journey shared with Verify milestones and publish completed Execution, including saved affected-work references. | None | Not yet met. |
