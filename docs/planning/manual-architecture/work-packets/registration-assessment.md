...
Purpose: Prepare and independently review a registration candidate.
...
Bind full/partial scope and exact selections; architect assessment, completion mapping, package/response validators and actual independent fidelity/correction loop.

Provider contract: Registration record/schema bundle owns package meanings and immutable candidate IDs; agent output hashes/claims are checked by service.

Consumer connection: Confirmation consumes only eligible reviewed candidate; source intake and live question flow supply both role selections before launch.
...
- [Exact source intake](source-intake.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/planning/registration.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/planning/registration_records.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/planning/test_registration_assessment.py` | Python | Real main-path and essential-failure checks |
...
1. Run actual architect assessment and non-author fidelity review on the same pinned inventory; validate package records, outcome references and bounded amendment counts.
2. Missing essential prerequisites, unsupported paths/hashes, invented approval and exhausted material review block readiness; no graph/run is activated.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/planning","-p","test_registration_assessment.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Missing essential scope/dependency or exhausted material review blocks candidate readiness; no execution work is created.
