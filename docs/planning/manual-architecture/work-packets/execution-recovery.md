...
Purpose: Recover interrupted delivery and record genuine completion.

Project outcomes: [EXE-PM5 — Pause, stop and recover Execution](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/execution-milestones.md#exe-pm5--pause-stop-and-recover-execution); [EXE-PM4 — Verify milestones and publish completed Execution](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/execution-milestones.md#exe-pm4--verify-milestones-and-publish-completed-execution).
...
Reconcile every journal/queue/session/QA stage; retain budgets and typed actions; completion requires all declared gates and publication evidence.

Provider contract: Stage modules each supply reconciliation callbacks from first delivery; this packet joins cross-stage recovery and final completion predicate.

Consumer connection: Monitoring renders saved blocker/available action; correction counts and original startup snapshots are retained.
...
- [Milestone promotion](promotion.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
- [Missing specialist support](specialist-support.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/execution/recovery.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/execution/completion.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_execution_recovery.py` | Python | Real main-path and essential-failure checks |
...
1. Reconcile all saved sessions/queues/Git/QA operations after restart and record completion only with every required exact gate/publication artifact.
2. Exercise restart at unknown external effect, exhausted budget, quarantined workspace and missing required artifact; none may silently complete.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/execution","-p","test_execution_recovery.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Restart never resets counts or accepts unknown remote effect; cancelled/paused/UNTESTED work cannot auto-complete.
