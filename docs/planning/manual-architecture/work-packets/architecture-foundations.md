...
Purpose: Save investigated structure and specialist records.
...
Implement bounded source investigation output, decisions, current/intended structure, role/context ownership and foundations manifest validation.

Provider contract: Foundation output includes exact assigned metadata; role context requires Verified facts, Source references and Knowledge gaps headings.

Consumer connection: Breakdown builds on these records, preserving unchanged versions and carried-forward provenance.
...
- [Architecture session entry](architecture-entry.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/planning/architecture_foundations.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/planning/test_architecture_foundations.py` | Python | Real main-path and essential-failure checks |
...
1. Validate/save investigation, decisions, current/intended structure and source-local role/context records, then publish a foundations manifest.
2. Reject context-owner collision, wrong headings/path, stale code source and missing foundations output; no full breakdown claim at this stage.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/planning","-p","test_architecture_foundations.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Unexpected path, colliding context owner or stale source rejects; saved foundations do not imply a complete breakdown.
