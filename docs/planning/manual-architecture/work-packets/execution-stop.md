...
Purpose: Settle current work and honor Owner disposition.

Project outcomes: [EXE-PM5 — Pause, stop and recover Execution](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/execution-milestones.md#exe-pm5--pause-stop-and-recover-execution).
...
Implement pause/resume/stop API/CLI, saved lifecycle intent and checkpoints at dispatch/review/integration/import/QA/promotion boundaries.

Provider contract: Lifecycle exposes service-owned admission/settlement checks consumed by every stage; it is delivered before live dispatch.

Consumer connection: Recovery uses saved intent/operation state; cancellation is not successful completion or budget reset.
...
|---|---|---|
| `services/maestro/maestro/execution/lifecycle.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_execution_stop.py` | Python | Real main-path and essential-failure checks |
...
1. Save Owner pause/resume/stop intent and enforce settlement barriers around launch/review/merge/import/QA/promotion; retain resumable checkpoints.
2. Race stop with each irreversible operation; reconcile started work, prohibit new dispatch and prevent stale-input revival on resume.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/execution","-p","test_execution_stop.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Stop racing an in-flight operation reconciles it; resume cannot duplicate work or revive stale inputs.
