...
Purpose: Bind each process to exact installed policy and schemas.

Project outcomes: [SVC-PM5 — Apply shared process definitions](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm5--apply-shared-process-definitions).
...
Implement definition validation, immutable effective settings, handler registry and shared bookkeeping with process-specific rules.

Provider contract: Shared policy registry takes process-local validators, migration sets, routes and required outputs. Process-specific defaults/meaning are not inferred from the registry.

Consumer connection: Registration, architecture and Execution each own their schema source and adapter; install package-resource discovery must retain required older versions.
...
|---|---|---|
| `services/maestro/maestro/service/processes.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/service/resources.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/service/test_process_policy.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/maestro/service/schema_registry.json` | JSON Schema or schema registry | Current process format/resource definition, with validation tests |
...
1. Load the documented process definitions and installed bundle resources; validate and snapshot effective settings/hash before activity creation; only supported policies dispatch.
2. Reject old keys, unknown fields, missing or altered bundle; restart uses the saved bundle/hash/settings and preserves counters while status remains readable.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/service","-p","test_process_policy.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Missing/mismatched resource or invalid duration blocks the affected activity; restart retains its snapshot.
