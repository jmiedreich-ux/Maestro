...
Purpose: Recover and replan architecture from confirmed inputs.
...
Implement publication/cancel/retry recovery, stale inputs and later-registration invalidation/reconciliation under retained counts.

Provider contract: Process recovery owns current/working/confirmed reference settlement and links decisions/questions to exact inputs.

Consumer connection: Execution start must reject stale architecture input; ordinary recovery is not automatic re-registration or scope change.
...
- [Architecture review and confirmation](architecture-confirmation.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/planning/architecture_recovery.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/planning/test_architecture_recovery.py` | Python | Real main-path and essential-failure checks |
...
1. Resume from verified saved references after interruption; reconcile publication/cancellation and later confirmed-registration change without inventing new authority.
2. Unconfirmed re-registration does not replan; stale input and missing published bytes block affected output; counts survive replacement.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/planning","-p","test_architecture_recovery.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Unconfirmed replacement registration does not trigger replan; restart neither resets counts nor guesses receipts.
