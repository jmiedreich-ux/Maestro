...
Purpose: Show projects, activities, history and attention.

Project outcomes: [CLI-PM1 — Connected multi-project CLI workspace](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/cli-milestones.md#cli-pm1--connected-multi-project-cli-workspace).
...
Implement selected-project workspace, scrolling/input/key behavior, activities and empty/error states from service records.

Provider contract: Renderer consumes typed projections; extension registration receives commands/views, not direct storage or authority.

Consumer connection: Linked questions, registration, architecture and Execution extensions add their own commands without editing this renderer's core.
...
|---|---|---|
| `services/maestro/maestro/terminal/workspace.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/terminal/rendering.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/terminal/test_terminal_workspace.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/maestro/terminal/extensions.py` | Python | Assigned behavior and integration contract |
...
1. Use the actual terminal renderer with service snapshots/events: select projects/activities, page history, edit input, scroll messages and see attention/empty/error states.
2. Exercise project switch with unsent input and connection loss while viewing history; preserve only architecture-permitted state and show explicit stale/offline status.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/terminal","-p","test_terminal_workspace.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Disconnect and project switch preserve only permitted context; errors cannot look like empty success.
