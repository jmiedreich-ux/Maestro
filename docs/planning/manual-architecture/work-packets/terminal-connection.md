...
Purpose: Connect the installed terminal to the service.

Project outcomes: [CLI-PM1 — Connected multi-project CLI workspace](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/cli-milestones.md#cli-pm1--connected-multi-project-cli-workspace); [SVC-PM3 — Connect the CLI to recorded service activity](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm3--connect-the-cli-to-recorded-service-activity).
...
Implement config/fallback, credential loading, fixed timeouts, reconnect and offline help/exit.

Provider contract: Connection module is the sole credentialed loopback HTTP/SSE client; expose receipt lookup, configuration reload and connection-state notifications.

Consumer connection: Workspace renders connection state; request submission and question extensions use one client, never direct SQL.
...
|---|---|---|
| `services/maestro/maestro/terminal/connection.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/terminal/main.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/terminal/test_terminal_connection.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/maestro/terminal/__init__.py` | Python | Package boundary and explicit exports |
...
1. Launch maestro as a service client; display effective address and auth/setup state. Exercise valid configured URL and malformed-config fallback at defined timeouts.
2. Initial failure waits for retry; later disconnect uses bounded backoff. A valid unreachable URL stays selected; changing address clears old input and never replays commands.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/terminal","-p","test_terminal_connection.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Unreachable valid URL does not fall back; changed address clears prior context; no automatic command replay.
