...
Purpose: Continue persistent sessions from saved evidence.

Project outcomes: [SVC-PM4 — Run and recover assigned agents](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm4--run-and-recover-assigned-agents); [ARC-PM1 — Establish the project's architectural foundations](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/architecture-loop-milestones.md#arc-pm1--establish-the-projects-architectural-foundations).
...
Implement session creation/resume/replacement, serialized actions, occupancy, context thresholds/checkpoints and accurate usage/performance records.

Provider contract: Session service separates idle session time from active runs and records tool-specific measurement provenance.

Consumer connection: Architecture and persistent managers consume this; registration fresh conversations retain only its specified capacity continuation exception.
...
|---|---|---|
| `services/maestro/maestro/agents/sessions.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/agents/measurements.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/agents/test_sessions.py` | Python | Real main-path and essential-failure checks |
...
1. Create and resume a persistent exact session with one action at a time, saved checkpoints, occupancy and measured usage; capacity continuation resumes verified work.
2. Cross warning/handoff/re-entry thresholds with controlled readings; preserve active-time/counts. Unknown capacity and lost session trigger the defined classification/replacement, not fake performance.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/agents","-p","test_sessions.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Capacity continuation preserves remaining active-time and budgets; unknown/stale measurements remain labeled.
