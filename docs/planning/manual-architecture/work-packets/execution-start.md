...
Purpose: Start one eligible Execution with exact snapshots.
...
Implement execution@1 schemas/records/API/CLI, explicit start, baseline/master distinction, settings and plan/catalog bindings.

Provider contract: Own execution@1 schema source, process records, start/status API and validator integration with common handler registry.

Consumer connection: Manager plans against saved startup inputs; lifecycle owns pause/stop. Exact code baseline and current master are separate fields.
...
- [Architecture reconciliation](architecture-recovery.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/execution/contracts.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/execution/start.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_execution_start.py` | Python | Real main-path and essential-failure checks |
| `schemas/execution.schema.json` | JSON Schema or schema registry | Current process format/resource definition, with validation tests |
| `services/maestro/maestro/execution/__init__.py` | Python | Package boundary and explicit exports |
| `services/maestro/maestro/execution/plugin.py` | Python | Assigned behavior and integration contract |
...
1. Explicit typed start selects the permitted manager route and atomically saves current confirmed inputs, baseline, observed master, config/schema/catalog snapshots and one activity.
2. Duplicate start opens existing eligible work; stale/missing confirmations, resource binding or schema reject before dispatch.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/execution","-p","test_execution_start.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Ineligible or duplicate start cannot duplicate dispatch; changed settings do not replace startup snapshot.
