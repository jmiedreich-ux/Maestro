...
Purpose: Supply reproducible real-journey setup and evidence capture.
...
Implement versioned test-project input generators, CLI drivers, failure injection at actual boundaries, artifact collection and cleanup for all declared journeys.

Provider contract: Tooling identifies generated input origin and uses actual public actions, plus controlled interruption seams for essential failure tests.

Consumer connection: Earlier packet tests provide their own bounded checks; this harness aggregates full-product milestone evidence and does not make early checks depend on the last milestone.
...
|---|---|---|
| `services/maestro/maestro/quality/acceptance.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/quality/test_acceptance_tooling.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/qa/run_acceptance.py` | Python | Assigned behavior and integration contract |
| `services/maestro/qa/README.md` | Markdown | Assigned behavior and integration contract |
| `services/maestro/qa/generate_project.py` | Python | Assigned behavior and integration contract |
...
1. Provide reusable real-project inputs and command-array drivers that exercise registration through completed Execution against the exact declared setup.
2. Driver fails or reports UNTESTED if service/agent/Git/reviewer/result path is bypassed; never manufacture confirmation, approval or completion rows.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/quality","-p","test_acceptance_tooling.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Generator supplies inputs only; missing actual agent/service/Git path remains UNTESTED.
