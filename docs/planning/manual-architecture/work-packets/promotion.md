...
Purpose: Promote only the exact passing milestone head.

Project outcomes: [EXE-PM2 — Integrate work and deliver declared dependencies](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/execution-milestones.md#exe-pm2--integrate-work-and-deliver-declared-dependencies); [EXE-PM4 — Verify milestones and publish completed Execution](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/execution-milestones.md#exe-pm4--verify-milestones-and-publish-completed-execution).
...
Journal branch containment/order/review/QA/artifact checks, code delivery and verified remote master result under authorized merge rules.

Provider contract: Promotion operation binds milestone branch source, expected master and authorized merge rule, with code and completion-record phases kept distinct.

Consumer connection: Completion closes only after required promotions and records verified; no extra Owner merge gate is added.
...
- [Milestone gaps and correction](milestone-gaps.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/execution/promotion.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_promotion.py` | Python | Real main-path and essential-failure checks |
...
1. Confirm exact passing milestone head, QA/artifact/outcome references and provider containment, then journal authorized remote product-master promotion.
2. Changed source/target/dependency or unknown remote outcome blocks/reconciles before retry; never approve moved code from old checks.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/execution","-p","test_promotion.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Moved head or changed dependency invalidates affected checks; uncertain promotion reconciles before retry.
