...
Purpose: Deliver committed progress and replay.

Project outcomes: [SVC-PM3 — Connect the CLI to recorded service activity](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm3--connect-the-cli-to-recorded-service-activity); [CLI-PM1 — Connected multi-project CLI workspace](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/cli-milestones.md#cli-pm1--connected-multi-project-cli-workspace).
...
Implement authenticated SSE, stable cursor, snapshots, heartbeat and missing-cursor recovery.

Provider contract: SSE uses the committed outbox/cursor contract, not agent stdout; connection heartbeats create no conversation or database event.

Consumer connection: Terminal connection/workspace owns reconnection display and duplicate filtering; processes only emit committed events.
...
|---|---|---|
| `services/maestro/maestro/service/events.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/service/test_events.py` | Python | Real main-path and essential-failure checks |
...
1. Connect an authenticated stream after reading a snapshot; receive intervening committed events once, with durable IDs and required heartbeat timing.
2. Reconnect with Last-Event-ID, duplicate delivery and an unavailable cursor; assert replay or fresh-snapshot requirement, not fabricated current state.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/service","-p","test_events.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: No event before commit; duplicate events ignored by consumer; unavailable cursor requests refresh.
