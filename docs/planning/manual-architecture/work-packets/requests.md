...
Purpose: Save requests before acknowledging work.

Project outcomes: [SVC-PM2 — Preserve project activity and requests](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm2--preserve-project-activity-and-requests); [SVC-PM3 — Connect the CLI to recorded service activity](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm3--connect-the-cli-to-recorded-service-activity).
...
Implement /api/v1 receipts, idempotent operations, expected-version rejection and uncertain-submit lookup.

Provider contract: A fixed registry maps operation names to handler validators and atomic command callbacks; request identity is independent of project/activity identity.

Consumer connection: Process modules register only their approved operation names; read projections and terminal reconciliation consume durable receipts.
...
|---|---|---|
| `services/maestro/maestro/service/requests.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/service/receipts.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/service/test_requests.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/maestro/service/registry.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/service/http.py` | Python | Assigned behavior and integration contract |
...
1. Submit a real registered test handler through /api/v1/requests; save its receipt and effect before returning accepted/completed. A request-status lookup retrieves that same identity.
2. Drop the response after commit, then query the same request ID; no duplicated side effect. Reuse the ID with different payload and assert 409.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/service","-p","test_requests.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Same request with different payload conflicts; dropped response reconciles without repeating the action.
