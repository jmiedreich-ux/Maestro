...
Purpose: Resolve missing specialist inputs within confirmed direction.
...
Select existing role or publish independently reviewed role/context additions via bounded support assignments; inherited config/counts and typed exhaustion handling.

Provider contract: Support output obeys source-local role/context ownership, exact reviewed version and configured fallback/limit rules.

Consumer connection: Reservations wait for confirmed specialist input; existing unchanged role selection needs no new content review.
...
|---|---|---|
| `services/maestro/maestro/execution/specialist_support.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_specialist_support.py` | Python | Real main-path and essential-failure checks |
...
1. Resolve a missing role by exact existing input or new bounded role/context with independent fidelity review, publication and validated activation.
2. Scope/direction change uses recorded Owner work disposition and re-registration; changed config cannot replace inherited snapshot or reset counts.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/execution","-p","test_specialist_support.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Scope/direction change uses Owner disposition and re-registration; new name cannot reset review counts.
