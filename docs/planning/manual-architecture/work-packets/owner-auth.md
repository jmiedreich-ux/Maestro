...
Purpose: Authenticate the configured local Owner.

Project outcomes: [SVC-PM1 — Operate the persistent Maestro service](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm1--operate-the-persistent-maestro-service); [SVC-PM3 — Connect the CLI to recorded service activity](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/runtime-service-milestones.md#svc-pm3--connect-the-cli-to-recorded-service-activity).
...
Implement installation-token digest validation, authority derived from the credential, rotation and read/write protection.

Provider contract: Authentication returns a verified actor or typed HTTP rejection before handler dispatch. Installer supplies protected token and digest configuration; it does not receive token from a model.

Consumer connection: Durable request delivery applies this boundary to both reads and writes; Linux installation owns generated test/production setup separation.
...
|---|---|---|
| `services/maestro/maestro/service/authentication.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/service/test_owner_auth.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/maestro/service/__init__.py` | Python | Package boundary and explicit exports |
...
1. Send a request with a valid test token and spoofed body identity; the saved actor is the configured Owner. Replace the credential and prove old requests lose authorization without altering historical actors.
2. Exercise missing/wrong token, non-Owner permission and forbidden redirect destination; assert secrets absent from logs/events/arguments.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/service","-p","test_owner_auth.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Missing/invalid token is 401; wrong authority is 403; body actor cannot impersonate Owner.
