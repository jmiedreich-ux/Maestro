...
Purpose: Deliver declared provider code before releasing consumers.
...
Journal early/completed-provider imports, exact commits, closure/hash, destination containment, invalidation and continuation after provider completion.

Provider contract: Dependency record binds provider source commit, consumer, closure, import journal and promotion dependencies.

Consumer connection: Reservations and milestone promotion use delivered readiness; imports remain FIFO Integration Manager work.
...
|---|---|---|
| `services/maestro/maestro/execution/dependencies.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/execution/imports.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_dependencies.py` | Python | Real main-path and essential-failure checks |
...
1. Import exact declared provider revisions/closure into consumer milestone branches and record verified destination containment before releasing work.
2. Test completed provider after held early import; moving master, approval flags or unrelated code cannot replace exact delivery; changed inputs invalidate affected downstream readiness.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/execution","-p","test_dependencies.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Approval or completion flag alone cannot unblock; unrelated code/master tip cannot replace declared provider.
