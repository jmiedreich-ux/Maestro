...
Purpose: Keep verifiable evidence through completion and retention.
...
Implement run-owned capture, size/secret checks, hashing, atomic publish, authorized retrieval, retention and tombstones.

Provider contract: SQL owns artifact path/hash/media/size/retention state; secrets never enter public records or repository reports.

Consumer connection: QA runner and completion validate required artifacts; retention job follows configured post-close days and recorded deletion identity.
...
|---|---|---|
| `services/maestro/maestro/quality/artifacts.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/quality/retention.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/quality/test_qa_artifacts.py` | Python | Real main-path and essential-failure checks |
...
1. Capture run-owned artifact to temporary file, enforce size/secret limits, hash and atomically publish, then authorize retrieval via project Owner.
2. Interrupt capture, exhaust quota and alter/miss required artifact; record tombstone/UNTESTED correctly and preserve evidence until Execution closes.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/quality","-p","test_qa_artifacts.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Missing/mismatched required evidence invalidates check; interrupted write, full storage and cleanup failure recorded.
