...
Purpose: Validate packets, milestones and QA-plan records together.

Project outcomes: [ARC-PM2 — Produce a bounded and parallel-ready work breakdown](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/architecture-loop-milestones.md#arc-pm2--produce-a-bounded-and-parallel-ready-work-breakdown).
...
Extend supplied schema, allocation/output inventories, execution requirements, qa_plan_ref and QA-plan producer/consumer validation; graph/outcome/parallel checks.

Provider contract: Own execution_requirements, packetDeliverable, qa_plan_ref and QA-plan definitions/allocations in the architecture-loop bundle.

Consumer connection: Architecture confirmation and Execution consumption must use the delivered schema, never accept missing fields by the older bundle.
...
- [Architecture foundations](architecture-foundations.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/planning/breakdown.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/planning/breakdown_validation.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/planning/test_breakdown_records.py` | Python | Real main-path and essential-failure checks |
| `docs/schemas/architecture-loop.schema.json` | JSON Schema or schema registry | Current process format/resource definition, with validation tests |
...
1. Validate complete packet/milestone/QA records, dependency graph, full registered-outcome mapping and safe parallel boundaries; extend actual producer schema/output inventories.
2. Reject cycles, missing required outputs/QA references, insufficient packet requirements, stale outcome links and invalid catalog selections before publication.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/planning","-p","test_breakdown_records.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Cycle, missing output, stale outcome, missing required capability/QA reference or invalid catalog selection blocks publication.
