...
Purpose: Review and confirm an exact development breakdown.

Project outcomes: [ARC-PM3 — Review and confirm the development breakdown](https://github.com/jmiedreich-ux/Maestro/blob/182bf4290ef2960d4691a7f41315ebf71d5bf85c/docs/milestones/architecture-loop-milestones.md#arc-pm3--review-and-confirm-the-development-breakdown).
...
Implement actual independent review, amendments, content inventory/hash, publication, exact confirmation and CLI view/actions.

Provider contract: Architecture terminal extension and service handlers operate on exact working versus confirmed references and shared publication journal.

Consumer connection: Recovery retains manifest/history/receipt and counts, and later re-registration invalidates affected references.
...
- [Breakdown schema and validation](breakdown-records.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/planning/architecture_confirmation.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/planning/test_architecture_confirmation.py` | Python | Real main-path and essential-failure checks |
| `services/maestro/maestro/terminal/architecture.py` | Python | Assigned behavior and integration contract |
...
1. Run independent review on exact full content inventory, retain amendments/affected coverage, publish and accept only the Owner-confirmed eligible hash.
2. Changing content invalidates affected review; stale confirmation or missing schemas/catalog evidence rejects; no automatic Execution.
...
```json
["python","-m","unittest","discover","-s","tests/maestro/planning","-p","test_architecture_confirmation.py","-v"]
```

Required assertions: completion criteria 1 and 2, plus this essential rejection/recovery boundary: Changing reviewed content invalidates affected approval; no confirmation of old/schema-invalid output; no Execution auto-start.
