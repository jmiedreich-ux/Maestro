...
Purpose: Obtain non-author review of the exact packet revision.
...
Separate deterministic checks from actual reviewer judgment; original-author correction, configured counts and exhaustion recommendation/Owner decision.

Provider contract: Review handler separates deterministic readiness from judgment; typed architect recommendation/Owner grant is validated and consumed once.

Consumer connection: Integration queue consumes approval evidence; determinations handles scope questions and exhausted-limit routing.
...
- [Coder plan and result delivery](coder-delivery.md): its listed provider contract must be integrated at an exact recorded commit before this packet uses it.
...
|---|---|---|
| `services/maestro/maestro/execution/reviews.py` | Python | Assigned behavior and integration contract |
| `services/maestro/maestro/execution/review_limits.py` | Python | Assigned behavior and integration contract |
| `tests/maestro/execution/test_packet_review.py` | Python | Real main-path and essential-failure checks |
