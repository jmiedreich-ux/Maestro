...
Compare feature dispatch-to-accepted-result time, including waiting and correction time, where saved events permit it. Identify whether late scope discovery, missing prerequisites, implementation defects, or review delay caused the longest extensions. Unknown durations stay unknown; use the result to adjust the next feature plan, not to reopen a passing milestone.
