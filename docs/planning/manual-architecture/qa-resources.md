...
| Service repository identity | GitHub App `Maestro Coordinator`; slug `maestro-coordinator`; App ID `4746601`; installation ID `157167451`; account `jmiedreich-ux`; installation active with all-repository selection. A fresh installation token successfully read `Maestro-qa`. | Before M3 QA, the service must prove with a fresh App token the configured `Contents: write` and `Administration: read` permissions, exact App/installation identity and configured profile mapping without exposing the key or token. |
| GitHub destination-policy QA targets | None yet. M3 requires two operator-prepared, run-owned QA branches in `Maestro-qa`: one allowed unprotected branch and one distinct protected or ruleset-bound branch. They must remain in the same configured repository/profile; no test creates, alters or bypasses policy. | Prepare and record the exact branch names, current rules/protection observation identities and profile binding before M3 QA. Missing either target or required App permission is UNTESTED and blocks M3 acceptance. A permission-denied target may be supplied as additional evidence. |
