...
| Service repository identity | GitHub App `Maestro Coordinator`; slug `maestro-coordinator`; App ID `4746601`; installation ID `157167451`; account `jmiedreich-ux`; installation active with all-repository selection. On 2026-09-19 the approved installation token read `Maestro-qa` contents and branch refs; it observed the authorized branch-protection endpoint as HTTP 404 and the protected branch endpoint as HTTP 200. The App has `Contents: write` and `Administration: write`; no secret or token value is retained. | The service rechecks the exact App/installation identity, fresh Contents-write and Administration-read access and configured profile mapping before each M3 QA run. |
| GitHub destination-policy QA targets | Run-owned branch `maestro-m3-authorized` is unprotected. Run-owned branch `maestro-m3-protected` is protected by the required status context `maestro-m3-qa-policy-sentinel`. Fresh Maestro Coordinator installation-token reads observed HTTP 404 and HTTP 200 respectively from the exact branch-protection endpoint. | Recheck the exact target names, App/installation identity, policy observation and profile binding before each M3 QA run. No test creates, alters or bypasses policy. |
