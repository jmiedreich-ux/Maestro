...
Each milestone needs all allocated packet implementations and independent reviews, exact integrated revisions, passing assembled QA for its claimed contribution, retained evidence and resolved in-scope material gaps under existing limits. Promotion follows the architecture's eligibility and authorized merge rules; no new Owner merge-approval gate is added. After the verified master merge, the Coordinator completes the delivery-host release and evidence finalization in the architecture before another development milestone starts. An unfinished finalization blocks later work but does not change verified merge evidence. Project declarations remain the authority for complete outcomes. Early milestones supply partial contributions where a declaration requires later domains; only the final outcome reconciliation can claim every selected declaration complete. Nothing here records executed checks.

## Manual retrospective between milestones

After delivery-host finalization and after each promoted milestone, the Coordinator saves a [manual milestone retrospective](milestone-retrospective.md) before another development milestone starts. It compares the delivered revision and evidence with the milestone's pinned architecture and requirements, then records architecture/delivery gaps and a separate plain lessons-learned list. It uses measured lifecycle data where available and labels unavailable time as unknown. This is not another approval gate, review round or correction allowance; it does not reopen completed scope. Material changes follow the existing architecture and Owner authority.
