...
| Execution class | umbrella is not executable; M1-03A/B use `codex-cloud-maestro-developer`; B's real qualification additionally requires Owner-accepted external setup |
| Typed upstream dependencies | unsatisfied `hard: MAESTRO-M1-02C-CUMULATIVE-INTEGRATION-PROOF @ ProjectArchitectAccepted`; B also depends on accepted A, and B qualification on `M1-03-EXTERNAL-SETUP @ OwnerAccepted` recorded by the Project Architect |
| Soft dependencies | none |
...
| Owned domains | additive action/observation schema; repository authority/payload contracts; local Git adapter; provider-neutral GitHub adapter; redaction/secret-reference boundary; `tests/m1_03/`; final architecture/operations docs |
...
| Planning authorities | M0-D01, D02, D03, D05, D06, D11, D12, D14, D15; Master Plan; Agent Workforce Control Plane; original M1 roadmap; current handoff; planning source base `6b3a16b2e32973b133776faa5407ab815035db46` |
...
| Dispatchable transition | umbrella never becomes runtime `Dispatchable`; after dependency satisfaction, packet Decision Fidelity approval, and Project Architect release, only A may be released; B code waits for A acceptance and B qualification waits for Owner-accepted setup |
...
| Stable ID | Typed serial dependency | Exact branch / worktree / base | Outcome and non-goals | Exact owned paths | Proof and routes | Locks / envelope / acceptance |
|---|---|---|---|---|---|---|
| `MAESTRO-M1-03A-LOCAL-GIT-RECOVERY` | unsatisfied `hard: MAESTRO-M1-02C-CUMULATIVE-INTEGRATION-PROOF @ ProjectArchitectAccepted` | `implementation/m1-03a-local-git-recovery`; `/home/jeremy/Development/Maestro-m1-03a-implementation`; created only from exact accepted M1-02C head after reconciliation | Schema-5 action ledger, closed authority/payloads, local observe/branch/commit/push and restart reconciliation with real worktree/bare remote; no GitHub API, credential, network, external setup, create/register, or downstream action | bounded migration/factory changes in `services/maestro/maestro/storage.py`; safe payload/event registration in `services/maestro/maestro/operational_state.py`; bounded shared primitives in `services/maestro/maestro/git_repository.py`; `services/maestro/maestro/repository_adapter.py`; exact cumulative version-only update in `tests/m1_01/test_project_authority_storage.py`; `tests/m1_03/test_action_storage.py`; `tests/m1_03/test_repository_adapter.py`; `tests/m1_03/test_repository_recovery.py` | Packet proofs 1-9, local portions of 14-15/18, all prior regressions/compileall; Developer -> Integration validate-only unless assembly -> fresh exact-range independent review | `shared:sqlite-schema`; `file:services-maestro-storage`; `path:maestro-operational-state`; `path:maestro-git-adapter`; `path:tests-m1-03`; 150 minutes, one attempt, at most one correction; routine Project Architect A acceptance opens only B code |
| `MAESTRO-M1-03B-NONLIVE-GITHUB-INTEGRATION` | `hard: MAESTRO-M1-03A-LOCAL-GIT-RECOVERY @ ProjectArchitectAccepted`; qualification additionally requires `hard: M1-03-EXTERNAL-SETUP @ OwnerAccepted` | `implementation/m1-03b-nonlive-github-integration`; `/home/jeremy/Development/Maestro-m1-03b-implementation`; created only from exact accepted A head; setup changes no code base | Provider-neutral GitHub transport and exact draft-PR/reviewer/check/review operations, offline contract proof, real dedicated non-live GitHub qualification, and cumulative evidence; no credential/repo provisioning, live target, merge/default/protection/admin/deploy/webhook | bounded integration in `services/maestro/maestro/repository_adapter.py`; `services/maestro/maestro/github_adapter.py`; `tests/m1_03/test_github_adapter.py`; `tests/m1_03/test_nonlive_github_integration.py`; `tests/m1_03/test_cumulative_contract.py`; `docs/architecture/m1-03-real-repository-and-github-adapter.md`; `docs/operations/m1-03-real-repository-and-github-adapter.md` | Packet proofs 9-19 and all A/prior gates; Developer -> Integration -> fresh exact-range review -> Owner-authorized setup -> attended real proof -> cumulative Integration/review -> routine Project Architect integrated acceptance | code locks `path:maestro-repository-adapter`, `path:maestro-github-adapter`, `path:tests-m1-03`, `file:m1-03-architecture-doc`, `file:m1-03-operations-doc`; 150 minutes, one attempt, at most one correction; qualification 90 minutes plus `external:owner-approved-m1-03-nonlive-github-repository` and `finite:owner-approved-m1-03-credential-session`; only integrated acceptance unlocks downstream |
