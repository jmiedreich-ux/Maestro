...
This graph materializes the already accepted M1–M4 roadmap into 26 bounded
...
polling, recovery, Integration, review, the normal M0-D05 correction, and an
eligible Project-Architect-authorized M0-D17 final correction are not Owner
decisions.
...
- committed in-scope work failing a named gate receives one exact normal
  correction; after its targeted terminal gates, only a result satisfying every
  M0-D17 fact may receive one Project-Architect-authorized final correction;
...
- a missing/infeasible contract, architecture defect, new failure class after
  correction, failed final correction, third correction, or exhausted
  allowance returns to the Project Architect;
...
| Planned location / role / model class | non-leased umbrella over A, non-executable B0, five serial B implementation worktrees, and C / dedicated Maestro Developer / session-inherited Codex model, with factual model and runtime recorded at each executable-slice preflight |
| Execution class | umbrella and B0 are not executable; M1-02A, B1-B5, and C each use `codex-cloud-maestro-developer` |
...
| Resource locks | acquired only by the current serial executable slice; see exact A/B1-B5/C nodes below |
| Input contract version | accepted M1-01 schema version `3` and `maestro-project-authority-load-result-v1` |
| Output contract version | Maestro operational schema version `5` (accepted A at 4 plus additive B correction carriers) and `maestro-operational-state-v1` |
| Planning authorities | M0-D01, D02, D03, D04, D05, D06, D11, D12, D14, D15, D16, and D17; Master Plan; Agent Workforce Control Plane; original M1 roadmap; planning source base `ed3d6cb2d2da03fbc5864f1727defcb2417f6e84` |
...
| Resource envelope | A 150 minutes; B1 90 minutes; B2-B5 120 minutes each; C 120 minutes; one Developer attempt, one normal correction and only an eligible M0-D17 final correction per executable slice; no third correction, overlapping slice, or locked-domain writer |
...
| Dispatchable transition | umbrella and B0 never become runtime `Dispatchable`; accepted A permits B0 planning validation; exact B0 Decision Fidelity approval and Project Architect release permits only B1 from base `d82164c2f3be2164ad6e66b022f645be5f61844b`; B2-B5 and C release serially only after exact predecessor acceptance |
...
|---|---|---|---|---|---|---|
| `MAESTRO-M1-02A-SCHEMA-RECORDS-VALIDATION` | satisfied `hard: MAESTRO-M1-01-REAL-PROJECT-AUTHORITY-LOADER @ ProjectArchitectAccepted` at `56b4dfb5e4d4bef860616cde93d172affb0e4210` | `implementation/m1-02a-schema-records-validation`; `/home/jeremy/Development/Maestro-m1-02a-implementation`; created only from exact base `56b4dfb5e4d4bef860616cde93d172affb0e4210`; accepted exact head `d82164c2f3be2164ad6e66b022f645be5f61844b` | Additive schema/records/closed validation, record/idempotency foundation, context/usage and M0-D11-safe store; no lifecycle/claim/recovery/final docs/external action | `services/maestro/maestro/storage.py`; `services/maestro/maestro/operational_state.py`; `tests/m1_02/test_schema_and_records.py`; `tests/m1_02/test_context_and_payloads.py`; and only the cumulative schema-version expectations in `tests/m1_01/test_project_authority_storage.py`: history `(2,3)` to `(2,3,4)` and `health.schema_version == 4`, with no removal, skip, weakening, or version-agnostic replacement | Proofs 1-9; proof 23 only for A-owned store/constructor/factory/record/read routes; proof 24 only for policy validation/digest/arithmetic/fit and both policy examples without lifecycle enforcement; proofs 25-29; Alpha/M1-01/compileall; Developer -> Integration validate-only unless assembly -> fresh exact-range independent review | `shared:sqlite-schema`; `path:maestro-operational-state`; `file:services-maestro-storage`; `path:tests-m1-02`; 150 minutes, one attempt, at most one correction; accepted Project Architect slice head opens only B0 planning validation |
| `MAESTRO-M1-02B0-CANONICAL-CONTRACT-FREEZE` | accepted exact A head `d82164c2f3be2164ad6e66b022f645be5f61844b` | non-executable planning carrier at `docs/planning/contracts/m1-02b-contract.json` | Sole canonical field/schema/API/event/relation/quality/requirement/proof source; no code or operational state | canonical JSON plus B1-B5 packet docs only | two independent ephemeral standard-library validation/mutation passes -> Decision Fidelity -> Project Architect release | never leased; release opens only exact-base B1 |
| `MAESTRO-M1-02B1-SCHEMA5-CARRIERS` | `hard: MAESTRO-M1-02B0-CANONICAL-CONTRACT-FREEZE @ ProjectArchitectReleased` | `implementation/m1-02b1-schema5-carriers`; `/home/jeremy/Development/Maestro-m1-02b1-implementation`; exact base `d82164c2f3be2164ad6e66b022f645be5f61844b` | Additive schema 4→5 correction carriers and canonical validation; no lifecycle action | `services/maestro/maestro/storage.py`; `services/maestro/maestro/operational_state.py`; `tests/m1_02/test_schema5_contract.py` | canonical `B1-P01..P03`; predecessor regressions; Developer -> Integration -> fresh exact-range review | schema/storage/state/test locks; 90 minutes; governed corrections; acceptance opens only B2 |
| `MAESTRO-M1-02B2-GUARDED-LIFECYCLE` | `hard: MAESTRO-M1-02B1-SCHEMA5-CARRIERS @ ProjectArchitectAccepted` | `implementation/m1-02b2-guarded-lifecycle`; `/home/jeremy/Development/Maestro-m1-02b2-implementation`; exact accepted B1 base resolved before release | Finite lifecycle and notification/acceptance/merge/head/context guards; one composite event per mutation; no claim/recovery/control action | `services/maestro/maestro/operational_state.py`; `tests/m1_02/test_lifecycle_transitions.py`; `tests/m1_02/test_acceptance_and_notifications.py` | canonical `B2-P01..P03`; all predecessor regressions; Developer -> Integration -> fresh exact-range review | state/test locks; 120 minutes; governed corrections; acceptance opens only B3 |
| `MAESTRO-M1-02B3-CLAIMS-LEASES-RECOVERY` | `hard: MAESTRO-M1-02B2-GUARDED-LIFECYCLE @ ProjectArchitectAccepted` | `implementation/m1-02b3-claims-leases-recovery`; `/home/jeremy/Development/Maestro-m1-02b3-implementation`; exact accepted B2 base resolved before release | Atomic claim/lease/locks, seams/contention and finite restart recovery; no redispatch/external action | `services/maestro/maestro/operational_state.py`; `services/maestro/maestro/recovery.py`; `tests/m1_02/test_claims_and_recovery.py` | canonical `B3-P01..P03`; all predecessor regressions; Developer -> Integration -> fresh exact-range review | schema/state/recovery/test locks; 120 minutes; governed corrections; acceptance opens only B4 |
| `MAESTRO-M1-02B4-CLOSED-COMPLETION-CONTROL` | `hard: MAESTRO-M1-02B3-CLAIMS-LEASES-RECOVERY @ ProjectArchitectAccepted` | `implementation/m1-02b4-closed-completion-control`; `/home/jeremy/Development/Maestro-m1-02b4-implementation`; exact accepted B3 base resolved before release | Completion/gate/finding aggregation, StandardCorrection #1, composite return/resolution and learning; no final correction | `services/maestro/maestro/operational_state.py`; `tests/m1_02/test_completion_control.py` | canonical `B4-P01..P04`; all predecessor regressions; Developer -> Integration -> fresh exact-range review | state/test locks; 120 minutes; governed corrections; acceptance opens only B5 |
| `MAESTRO-M1-02B5-DISCRETIONARY-FINAL-CORRECTION` | `hard: MAESTRO-M1-02B4-CLOSED-COMPLETION-CONTROL @ ProjectArchitectAccepted` | `implementation/m1-02b5-discretionary-final-correction`; `/home/jeremy/Development/Maestro-m1-02b5-implementation`; exact accepted B4 base resolved before release | M0-D17 gate/authorization, atomic final resume/attempt, hard correction limit and cumulative B proof; no C/downstream/external action | `services/maestro/maestro/operational_state.py`; `tests/m1_02/test_final_correction.py`; `tests/m1_02/test_b_cumulative_contract.py` | canonical `B5-P01..P04`; all B/prior gates, stress and contiguous review coverage; Developer -> Integration -> fresh exact-range review | state/test locks; 120 minutes; governed corrections; exact-head PA B acceptance opens only C |
| `MAESTRO-M1-02C-CUMULATIVE-INTEGRATION-PROOF` | `hard: MAESTRO-M1-02B5-DISCRETIONARY-FINAL-CORRECTION @ ProjectArchitectAccepted`, including cumulative B1-B5 proof and coverage | `implementation/m1-02c-cumulative-integration-proof`; `/home/jeremy/Development/Maestro-m1-02c-implementation`; exact accepted B5 base | Cumulative A+B integration, architecture/operations docs, all-gate proof and exact final head; no new schema/state/product semantics/downstream or external action | exact A/B code/tests plus `tests/m1_02/test_cumulative_contract.py`, architecture and operations docs; only cumulative corrections | Proofs 1-32 and all gates; Developer -> Integration -> fresh C exact-range review -> cumulative accepted-M1-01-through-C coverage | B locks plus documentation locks; 120 minutes; governed corrections; only routine PA integrated C acceptance creates accepted M1-02 |
