...
New: packaged bundles `schemas/registration-process/1` and `schemas/architecture-loop/1` (byte copies of `docs/schemas`, checked by a test); `service/process_definitions.py` (providers for both processes, handlers that turn a snapshot into start operation, review limit and assignment terms, per-process report); `GET /api/v1/processes` (Owner-authenticated, re-reads the file each call, an invalid process is held alone). The upgrade adds shipped bundles to `/opt/maestro/schemas` (never changing a published one, undone on rollback). `ProcessDefinitions.start_activity` saves the definition with a new activity and `assignment_terms` gives an assignment its limits from that saved definition. Registration package validation, publication, confirmation and Owner limit decisions stay with the registration and architecture-loop outcomes.
