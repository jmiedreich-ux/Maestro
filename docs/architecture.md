...
| Quality Assurance plan | `milestone_ref`, versioned `setup_steps`, `support_processes`, `environment_refs`, `secret_refs`, `allowed_network_dependencies`, `data_requirements`, `checks`, `artifact_requirements`, `cleanup_steps`, and `reset_check`. |
...
Effective review allowance is the snapshotted base limit plus recorded grants minus completed rounds. A consumed decision cannot grant another attempt under a different request ID. A later exhaustion may create another decision only while the configured target maximum permits it. Conflicting or stale submissions return 409 with current state. Work-disposition decisions change restrictions, not review or retry counts.

Registration uses this handler for fidelity-review grants; the architecture loop uses it for fidelity-review and output-correction grants; Execution uses it for its three review types, manual retry, and re-registration disposition. Registration response corrections retain their existing technical-recovery budget and `registration.retry` action. Cancellation, pause, stop and restart preserve every grant, reservation and consumption record.
