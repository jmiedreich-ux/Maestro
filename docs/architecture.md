...
Each installed route also names a non-secret settings profile with a nonempty, exact permitted-destination list. Every destination records the HTTPS hostname and port used by that provider route; wildcards and implicit destinations are invalid. The route configuration hash includes this list, so a changed destination policy invalidates preflight and cannot be substituted during a run. Credentials remain a separate service-only profile and are never written into this settings record. Before a tool starts, a root-owned fixed egress launcher resolves only that route’s configured hostnames, supplies the isolated runtime a generated read-only hosts file, and starts it in a transient systemd unit. The unit denies all IP destinations except those resolved addresses; a cgroup-specific firewall permits only the matching configured TCP address-and-port pairs and denies every other socket. If any part of that restriction cannot be installed, the tool does not start. The launcher is not an agent-controlled command path.
...
#### GitHub destination authorization

A GitHub repository binding uses one service-owned GitHub destination-authorization provider. Its executable non-secret profile is the `repositories.<profile>.github` configuration defined under [adapter configuration](#adapter-configuration): `api_base_url` (default `https://api.github.com`), `app_id`, `installation_id`, `app_slug`, repository and branch allowlists. `repositories.<profile>.credential_profile` identifies that App private key in the service credential store. The service alone creates a short-lived installation access token for the bound repository and uses that same bound identity for Git transport and API authorization. Agents, CLI requests, packet worktrees and durable repository records never receive the private key or token.
...
| `repositories.<profile>.credential_profile` | Provisioned service credential reference. For a GitHub profile it identifies only that App private key; foundation derives short-lived installation tokens and no caller receives the key or token. |
...
| `repositories.<profile>.github.api_base_url` | HTTPS GitHub API base URL; absent means `https://api.github.com`. It must be an explicit permitted destination. |
...
### Delivery stopping boundaries

A confirmed milestone has a fixed outcome and required evidence. Planning may refine later features as source changes, but it cannot silently add outcomes, relax required checks, or convert a roadmap idea into active work. An idea beyond confirmed scope is recorded for a later milestone; changing the current outcome follows re-registration and Owner work disposition. Existing confirmed packet versions, including work already in progress, remain binding until amended through their established authority.
...
Milestone Quality Assurance and independent outcome review decide whether the assembled milestone meets its agreed behavior and evidence. Once its required gates pass, follow the authorized promotion and completion path without keeping the milestone open for optional polish. Record time from feature dispatch to accepted connected result, including waiting and correction time, and classify blocking correction causes. The retrospective uses those observations to change the next planning walkthrough or focused check; missing measurements are recorded as unknown.
...
Agent-authored investigation files use `preparedInvestigation` in assignment `required_outputs.schema_ref`; their findings use response-local keys. The wrapper verifies the agent's exact-byte artifact hashes and prepared schema first. Response findings and artifact findings with the same local key must agree. It then resolves existing bindings or allocates new identities, validates the resulting `investigation` saved schema, and calculates new hashes for the saved bytes. The independent reviewer writes no file: its response carries the outcome, the exact reviewed set and its findings, and the service builds and validates the saved `review` record from them (reviewer keys are prefixed `review-` so they cannot collide with investigation keys). Prepared and saved hashes remain distinct in the operation journal; publication and later references use only the saved hash.
...
The published `architecture-loop@1` bundle (`docs/schemas/architecture-loop.schema.json`) cannot change once installed, so it does not represent work-packet `execution_requirements`, milestone `qa_plan_ref`, or Quality Assurance plan records and inventory entries from the [Architecture record contract](#architecture-record-contract). The installed `architecture-breakdown@1` bundle (`docs/schemas/architecture-breakdown.schema.json`) carries those additions with full copies of the shared definitions, and the service validates every saved breakdown record, its decisions snapshot and its manifest against it. Foundation records remain validated against `architecture-loop@1`. Review and confirmation records are validated against the same bundle. The `allocation_required` result is not yet supported: the service assigns record identities itself.
...
### Delivery-host release and evidence finalization

After a milestone has a verified merge into `master`, the Coordinator finalizes that exact revision before another development milestone begins. For a milestone that changes the installed service, CLI, installed schemas or installation behavior, finalization uses the approved installation procedure to put the exact master revision on the configured delivery host. It preserves configured credentials and Owner settings, confirms that the running service and terminal use the released revision, and performs the declared post-install health and smoke checks. A milestone with no installed-runtime change records that no delivery-host release is required.

Finalization creates one durable release receipt containing the milestone, remote master commit, installed revision or bundle hashes, command and result evidence, service health, and declared smoke evidence. A failed, unclear or mismatched release blocks the next milestone. It does not alter the verified merge or invent a passing runtime result. The Coordinator uses existing recovery and incident authority; it does not silently substitute a candidate worktree or deploy a different revision.

After the release receipt is durable, cleanup removes only run-owned temporary workspaces, isolated environments, transient test units or files, and disposable scripts. Final QA, outcome-review, integration and release evidence is retained at the declared location with a manifest binding it to the milestone and commits. Branches and worktrees remain until the activity closes and no recovery or audit needs them. Cleanup records each target and result; ambiguous or failed cleanup is quarantined, never replaced by broad deletion.
