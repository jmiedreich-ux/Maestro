...
The published `architecture-loop@1` bundle (`docs/schemas/architecture-loop.schema.json`) cannot change once installed, so it does not represent work-packet `execution_requirements`, milestone `qa_plan_ref`, or Quality Assurance plan records and inventory entries from the [Architecture record contract](#architecture-record-contract). The installed `architecture-breakdown@1` bundle (`docs/schemas/architecture-breakdown.schema.json`) carries those additions with full copies of the shared definitions, and the service validates every saved breakdown record, its decisions snapshot and its manifest against it. Foundation records remain validated against `architecture-loop@1`. Allocation and response inventories, and confirmation of a breakdown for Execution, remain with the review and confirmation outcome.
