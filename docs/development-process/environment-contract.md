...
| GitHub and test targets | Required read/write access to the actual repository and test destination; verify GitHub app permissions including Administration: read only where an operation requires it, and whether the intended test repository supports the required protected-branch case. The reported private QA repository cannot enable branch protection without GitHub Pro; use an eligible isolated target if that case is required. Ordinary write access is a separate check. |
...
| Install and upgrade | Installation and version procedure for the selected feature; exact revision, preservation of settings, upgrade and rollback path when applicable. No automatic installation is claimed until the delivery path is implemented and observed. |
