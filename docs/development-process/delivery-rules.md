...
## Outcome quality and completion record

These rules apply to every feature and outcome plan, whoever does the work.
...
- **Checklist and completion record.** Plan and verify from the whole outcome document: its capability and scope, common requirements, Included and Excluded lists, every architecture section it references, its dependencies, each acceptance criterion and its definition of done. Record the results in that outcome's own document, since outcome specifications own completion evidence. Add a Result column to its acceptance criteria table (met or accepted exception, date, implementation revision, what was observed in plain words, evidence location), and a short Completion record after the definition of done (implementation revision, reproducible setup, independent review, and the observed state of each Included item). An outcome closes only when no item is unchecked except accepted exceptions.

None of these records exists in advance. Feature plans, Result columns and Completion records are created when work reaches the outcome: write a feature plan in `docs/planning/features/` following the structure of an existing one, and add the Result column and Completion record to the outcome document. If an outcome document or section the work needs is missing or incomplete, write it from the roadmap and the architecture, and record the choices made, instead of waiting.
