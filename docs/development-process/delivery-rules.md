...
Each feature has one observable finish line, one owner and a connected implementation. Quality comes first: the outcome's observable result must actually work, exercised end to end with real agents, models and repositories; mocks, passing checks and file presence never count. The owner includes necessary wiring, essential errors and recovery. Packet notes are saved within the feature plan as progress and recovery context; they are not separate repository planning files. Delegation to other agents is explicit, and the owner still delivers the whole flow.

Before dispatch, use the existing implementation assessment. Reuse or adapt a working path whenever it can meet the feature's result; authorize replacement only for a recorded technical or behavioral incompatibility. A new feature should close the demonstrated gap, not recreate working code. Provide a short brief pointing to authoritative sources. Check the [single environment contract](environment-contract.md) with one preflight command. At each milestone start rerun its applicable checks and a small smoke check. If setup is absent, report the exact blocker; do not improvise a substitute or count it as a code defect. Use a known-good workspace and perform a small authorized feasibility check for an uncertain route. Plan and verify from the whole outcome document: its capability and scope, common requirements, Included and Excluded lists, referenced architecture sections, dependencies, each acceptance criterion and its definition of done. Record the results in that outcome document, which owns completion evidence: a Result column on its acceptance table (met or accepted exception, date, implementation revision, what was observed, evidence location) and a short Completion record after the definition of done (revision, reproducible setup, independent review, observed state of each Included item). None of this exists in advance; create feature plans in `docs/planning/features/` and these records when work reaches the outcome, and if an outcome document or section is missing or incomplete, write it from the roadmap and the architecture instead of waiting. An outcome closes only when no item is unchecked except accepted exceptions.
