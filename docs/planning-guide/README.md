...
For later development preparation, use the [staged planning and implementation walkthrough](../architecture.md#staged-planning-and-implementation-walkthrough). Keep the full outcome and milestone roadmap, then prepare detailed work for the next connected feature or small batch against current code. A coding-capable agent traces the draft implementation before the existing breakdown review; an uncertain operational assumption receives a small authorized feasibility check or remains unverified. The implementer receives a concise brief linked to the authoritative sources. This does not change registration input formats, create an extra review round, or make a partial roadmap eligible for Execution under the current schema. Apply the [delivery stopping boundaries](../architecture.md#delivery-stopping-boundaries): fix a milestone's agreed outcome, give each feature an observable finish line, and classify newly discovered work before it can extend delivery.
