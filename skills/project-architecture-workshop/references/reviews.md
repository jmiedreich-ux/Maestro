...
Do not supply the author's conclusions, expected verdict, suspected defects, prior review findings on the same content, or suggested fixes to a fresh reviewer. Start a separate context without the drafting conversation. If source files contain prior review results or readiness conclusions, exclude those passages from the input packet, record their original location and the filtered packet hash, and supply the remaining text unchanged. Prepare and inspect this packet before assigning reviewers; give access only to the filtered content, not an instruction to ignore conclusions after reading them. Preserve factual technical claims that are being tested, but remove claims that coverage is sufficient or a previous review passed. Do not hide requirements or deferrals when filtering conclusions. A reviewer who accidentally receives prohibited material cannot count as the fresh independent pass; record the limitation.
...
| Storage and transactions | Authoritative owner/writer, chosen engine and deployment model, engine-dependent concurrency guarantees, atomic boundaries, and durability behavior. If engine selection is deferred, cite the accepted deferral, decision owner and selection point, and show which guarantees remain unverified. A generic requirement for SQL or transactions is not evidence of an engine decision. |
...
Expand each trace category into its actual actors, choices, records and operations. One general reference does not cover every item: trace architect and reviewer selections separately, and follow a source reference from intake through publication and later use. For every selection, record separate evidence for its input source, choosing actor, collection point, saved value, validation and consuming operation. A statement that a value is selected or checked before use does not define how it is collected. An existence check or launch-time validation cannot fill a missing input step. Use separate coverage subitems rather than marking the entire selection chain defined from one general heading. For consistency, compare the literal configuration hierarchy, keys, defaults and units across processes, and each saved format, required heading, filename and path against its schema, template and consumer. Record an explicit mapping or accepted difference; do not infer one from similar purpose. Distinguish installed resource locations from repository source paths.
