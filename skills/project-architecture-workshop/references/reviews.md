...
Expand each trace category into its actual actors, choices, records and operations. One general reference does not cover every item: trace architect and reviewer selections separately, and follow a source reference from intake through publication and later use. For consistency, compare the literal configuration hierarchy, keys, defaults and units across processes, and each saved format, required heading, filename and path against its schema, template and consumer. Record an explicit mapping or accepted difference; do not infer one from similar purpose. Distinguish installed resource locations from repository source paths.
